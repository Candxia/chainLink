<template>
  <div class="p-5">
    ...
    <div class="flex justify-between mb-4"><h2 class="text-lg font-bold">Menu Management</h2><el-button type="primary" @click="handleAdd">Add</el-button></div>
    <el-table :data="list" border stripe v-loading="loading">
      <el-table-column prop="menu_name" label="Name" />
      <el-table-column prop="title" label="Title" />
      <el-table-column prop="path" label="Path" />
      <el-table-column prop="menu_type" label="Type" />
    ...
    <el-dialog v-model="dialog" :title="editId ? 'Edit' : 'Add'" width="500px">
      <el-form :model="form" label-width="80px">
        <el-form-item label="Name"><el-input v-model="form.menu_name" /></el-form-item>
        ...
        <el-form-item label="Type"><el-input v-model="form.menu_type" /></el-form-item>
        <el-form-item label="Parent ID"><el-input v-model="form.parent_id" /></el-form-item>
        <el-form-item label="Icon"><el-input v-model="form.icon" /></el-form-item>
        <el-form-item label="Order"><el-input v-model="form.order_num" /></el-form-item>
      </el-form>
...
import { getMenuListApi, addMenuApi, editMenuApi, delMenuApi } from '@/api/system'
...
const getList = async () => {
  loading.value = true
  try { const r = await getMenuListApi({ page: page.value, pageSize: pageSize.value, ...search }); list.value = r.data?.list || []; total.value = r.data?.total || 0 }
...
const handleSave = async () => {
  saving.value = true
  try { const fn = editId.value ? editMenuApi : addMenuApi; const r = await fn(editId.value ? { id: editId.value, ...form } : form); if (r.code === 0) { ElMessage.success('Saved!'); dialog.value = false; getList() } }
  catch(e){} finally { saving.value = false }
}
const handleDelete = async (id) => {
  try { await ElMessageBox.confirm('Delete?'); const r = await delMenuApi({ id }); if (r.code === 0) { ElMessage.success('Deleted!'); getList() } }
  catch(e){}
}
