<template>
  <div class="p-5">
    ...
    <div class="flex justify-between mb-4"><h2 class="text-lg font-bold">Dept Management</h2><el-button type="primary" @click="handleAdd">Add</el-button></div>
...
import { getDeptListApi, addDeptApi, editDeptApi, delDeptApi } from '@/api/system'
...
const getList = async () => {
  loading.value = true
  try { const r = await getDeptListApi({ page: page.value, pageSize: pageSize.value, ...search }); list.value = r.data?.list || []; total.value = r.data?.total || 0 }
...
const handleSave = async () => {
  saving.value = true
  try { const fn = editId.value ? editDeptApi : addDeptApi; const r = await fn(editId.value ? { id: editId.value, ...form } : form); if (r.code === 0) { ElMessage.success('Saved!'); dialog.value = false; getList() } }
  catch(e){} finally { saving.value = false }
}
const handleDelete = async (id) => {
  try { await ElMessageBox.confirm('Delete?'); const r = await delDeptApi({ id }); if (r.code === 0) { ElMessage.success('Deleted!'); getList() } }
  catch(e){}
}
